...
Historical effectiveness in this project: karen has caught fabricated gate results, false-green oracles with no real document-state assertion, inflated completion claims (wont-fix used to avoid achievable work), premature scope closures, criterion mismatches between pipeline.json and actual implementation, and hollow test labels disguising liveness checks as feature coverage. Every karen review has resulted in concrete improvements to the phase output. Findings are not adversarial — they are the next work items.
